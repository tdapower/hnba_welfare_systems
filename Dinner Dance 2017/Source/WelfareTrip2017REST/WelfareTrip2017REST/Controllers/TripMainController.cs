...
namespace WelfareTrip2017REST.Controllers
{
    public class TripMainController : ApiController
    {
        ...
        public TripMain GetMainDetailsByEpf(string company, int epf)
        {
            ...
            OracleCommand command;
            string sql = "SELECT " +
            "CASE WHEN t.SEQ_ID IS NULL THEN 0  ELSE t.SEQ_ID END, " + //0
            "CASE WHEN t.COMPANY IS NULL THEN ''  ELSE t.COMPANY END, " +//1
            "CASE WHEN t.BRANCH_DEPT IS NULL THEN ''  ELSE t.BRANCH_DEPT END, " +//2
            "CASE WHEN t.EPF_NO IS NULL THEN 0  ELSE t.EPF_NO END, " +//3
            "CASE WHEN t.MEMBER_NAME IS NULL THEN ''  ELSE t.MEMBER_NAME END, " +//4
            "CASE WHEN t.MARITIAL_STATUS IS NULL THEN ''  ELSE t.MARITIAL_STATUS END, " +//5
            "CASE WHEN t.SPOUSE_NAME IS NULL THEN ''  ELSE t.SPOUSE_NAME END, " +//6
            "CASE WHEN t.IS_MEMBER_PARTICIPATE IS NULL THEN 0  ELSE t.IS_MEMBER_PARTICIPATE END, " +//7
            "CASE WHEN t.IS_SPOUSE_PARTICIPATE IS NULL THEN 0  ELSE t.IS_SPOUSE_PARTICIPATE END, " +//8
            "CASE WHEN t.SYSTEM_DATE IS NULL THEN to_date('01/01/1900', 'DD/MM/RRRR')  ELSE to_date(t.SYSTEM_DATE, 'DD/MM/RRRR') END " +//9
            ...
            try
            {
                ...
                if (dataReader.HasRows)
                {
                    ...
                    tripMain.MaritialStatus = dataReader[5].ToString();
                    tripMain.SpouseName = dataReader[6].ToString();
                    tripMain.IsMemberParticipate = Convert.ToInt32(dataReader[7]);
                    tripMain.IsSpouseParticipate = Convert.ToInt32(dataReader[8]);
                    tripMain.SystemDate = dataReader[9].ToString();
        ...
        public HttpResponseMessage UpdateTripMainl(TripMain obj)
        {
            ...
            {
                connection.Open();
                command = new OracleCommand("UPDATE_WELF_DD_2017");
                command.CommandType = CommandType.StoredProcedure;
                command.Connection = connection;
                command.Parameters.Add("V_SEQ_ID", OracleType.Number).Value = obj.SeqId;
                command.Parameters.Add("V_IS_MEMBER_PARTICIPATE", OracleType.Number).Value = obj.IsMemberParticipate;
